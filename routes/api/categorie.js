...
router.get('/', async (req, res) => {
  try {
    const elements = await Categorie.find().populate('owner', ['name']);
    res.json(elements);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.get('/ById/:id', async (req, res) => {
  try {
    //remove type
    const element = await Categorie.findOne({ _id: req.params.id });
    res.json(element);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      element = await Categorie.findOneAndUpdate(
        { _id: req.params.id },
        { $set: Fields },
        { new: true }
      );
